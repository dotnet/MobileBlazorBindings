    ...
    public async processBatch(receivedBatchId: number, batchData: Uint8Array): Promise<void> {
        ...
        if (receivedBatchId > this.nextBatchId) {
            if (this.fatalError) {
                ...
                await DotNet.invokeMethodAsync('Microsoft.MobileBlazorBindings.WebView', 'OnRenderCompleted', this.nextBatchId - 1, this.fatalError.toString());
                return;
            }
            return;
        }
        ...
        try {
            ...
            DotNet.invokeMethodAsync('Microsoft.MobileBlazorBindings.WebView', 'OnRenderCompleted', receivedBatchId, error.toString());
            throw error;
        }
    }
